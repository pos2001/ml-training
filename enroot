

“Docker 이미지 빌드 → ECR 등록 → 헤드노드에서 pull → sqsh 변환”
ParallelCluster 컨테이너 기반 분산 트레이닝 완전 가이드

네, 제시하신 워크플로우가 맞습니다! 전체 프로세스를 처음부터 끝까지 보여드리겠습니다.

1단계: 컨테이너 이미지 준비

1.1 Dockerfile 작성

Dockerfile
FROM nvidia/cuda:11.8.0-cudnn8-devel-ubuntu22.04

기본 패키지 설치
RUN apt-get update && apt-get install -y \
    python3-pip \
    python3-dev \
    git \
    wget \
    && rm -rf /var/lib/apt/lists/*

PyTorch 및 분산 트레이닝 라이브러리 설치
RUN pip3 install --no-cache-dir \
    torch torchvision torchaudio \
    transformers \
    accelerate \
    deepspeed \
    pytorch-lightning \
    wandb

NCCL 및 MPI 설정
ENV NCCL_DEBUG=INFO
ENV NCCL_SOCKET_IFNAME=^docker0,lo

작업 디렉토리 설정
WORKDIR /workspace

트레이닝 스크립트 복사
COPY train.py /workspace/
COPY requirements.txt /workspace/

추가 의존성 설치
RUN pip3 install -r requirements.txt

엔트리포인트 설정
ENTRYPOINT ["python3"]
CMD ["train.py"]


1.2 로컬에서 이미지 빌드 및 테스트

이미지 빌드
docker build -t my-training-container:latest .

로컬 테스트
docker run --gpus all \
  -v $(pwd)/data:/workspace/data \
  -v $(pwd)/output:/workspace/output \
  my-training-container:latest train.py --test


1.3 ECR에 푸시

ECR 로그인
aws ecr get-login-password --region us-east-1 | \
  docker login --username AWS --password-stdin ${ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com

ECR 리포지토리 생성 (최초 1회)
aws ecr create-repository --repository-name ml-training

이미지 태깅
docker tag my-training-container:latest \
  ${ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/ml-training:latest

ECR에 푸시
docker push ${ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/ml-training:latest


2단계: ParallelCluster 헤드노드에서 컨테이너 준비

2.1 ECR에서 이미지 Pull 및 변환

#!/bin/bash
prepare_container.sh

set -e

ECR_URI="${ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com"
CONTAINER_NAME="ml-training"
TAG="latest"
FSX_CONTAINER_DIR="/fsx/containers"

echo "=== 1. ECR 로그인 ==="
aws ecr get-login-password --region us-east-1 | \
  docker login --username AWS --password-stdin ${ECR_URI}

echo "=== 2. 컨테이너 이미지 Pull ==="
docker pull ${ECR_URI}/${CONTAINER_NAME}:${TAG}

echo "=== 3. Enroot로 변환 ==="
임시 디렉토리 생성 (충분한 공간 확보)
mkdir -p /tmp/enroot
export ENROOT_TEMP_PATH=/tmp/enroot

sqsh 파일로 변환
enroot import -o /tmp/${CONTAINER_NAME}.sqsh \
  docker://${ECR_URI}/${CONTAINER_NAME}:${TAG}

echo "=== 4. FSx로 이동 ==="
FSx 디렉토리 확인 및 생성
sudo mkdir -p ${FSX_CONTAINER_DIR}
sudo chmod 755 ${FSX_CONTAINER_DIR}

컨테이너 이미지 이동
sudo mv /tmp/${CONTAINER_NAME}.sqsh ${FSX_CONTAINER_DIR}/

권한 설정
sudo chmod 644 ${FSX_CONTAINER_DIR}/${CONTAINER_NAME}.sqsh

echo "=== 5. 정리 ==="
rm -rf /tmp/enroot
docker rmi ${ECR_URI}/${CONTAINER_NAME}:${TAG}

echo "완료! 컨테이너 위치: ${FSX_CONTAINER_DIR}/${CONTAINER_NAME}.sqsh"


2.2 실행

스크립트 실행 권한 부여
chmod +x prepare_container.sh

환경 변수 설정 후 실행
export ACCOUNT_ID="123456789012"
./prepare_container.sh


3단계: 분산 트레이닝 스크립트 작성

3.1 Slurm 작업 스크립트

#!/bin/bash
#SBATCH --job-name=distributed-training
#SBATCH --nodes=4                    # 4개 노드 사용
#SBATCH --ntasks-per-node=8          # 노드당 8개 GPU (p4d.24xlarge 기준)
#SBATCH --cpus-per-task=12           # GPU당 CPU 코어
#SBATCH --gres=gpu:8                 # 노드당 GPU 수
#SBATCH --time=24:00:00              # 최대 실행 시간
#SBATCH --output=/fsx/logs/%j.out
#SBATCH --error=/fsx/logs/%j.err

환경 변수 설정
export CONTAINER_IMAGE="/fsx/containers/ml-training.sqsh"
export DATA_DIR="/fsx/data"
export OUTPUT_DIR="/fsx/output/${SLURM_JOB_ID}"
export CHECKPOINT_DIR="/fsx/checkpoints/${SLURM_JOB_ID}"

출력 디렉토리 생성
mkdir -p ${OUTPUT_DIR}
mkdir -p ${CHECKPOINT_DIR}

NCCL 설정 (EFA 활용)
export NCCL_DEBUG=INFO
export NCCL_PROTO=simple
export FI_PROVIDER=efa
export FI_EFA_USE_DEVICE_RDMA=1
export NCCL_SOCKET_IFNAME=^docker0,lo

마스터 노드 정보
export MASTER_ADDR=$(scontrol show hostname ${SLURM_NODELIST} | head -n 1)
export MASTER_PORT=29500

전체 프로세스 수
export WORLD_SIZE=$((SLURM_NNODES * SLURM_NTASKS_PER_NODE))

echo "=== Job Information ==="
echo "Job ID: ${SLURM_JOB_ID}"
echo "Nodes: ${SLURM_NNODES}"
echo "GPUs per node: ${SLURM_NTASKS_PER_NODE}"
echo "Total GPUs: ${WORLD_SIZE}"
echo "Master: ${MASTER_ADDR}:${MASTER_PORT}"
echo "======================="

Enroot/Pyxis를 통한 컨테이너 실행
srun --container-image=${CONTAINER_IMAGE} \
     --container-mounts=${DATA_DIR}:/data,${OUTPUT_DIR}:/output,${CHECKPOINT_DIR}:/checkpoints \
     --container-workdir=/workspace \
     python3 train.py \
       --data_dir /data \
       --output_dir /output \
       --checkpoint_dir /checkpoints \
       --batch_size 32 \
       --learning_rate 1e-4 \
       --num_epochs 100 \
       --distributed


3.2 PyTorch 분산 트레이닝 코드 예시

train.py
import os
import torch
import torch.distributed as dist
from torch.nn.parallel import DistributedDataParallel as DDP
from torch.utils.data.distributed import DistributedSampler

def setup_distributed():
    """분산 환경 초기화"""
    # Slurm 환경 변수에서 정보 가져오기
    rank = int(os.environ.get('SLURM_PROCID', 0))
    world_size = int(os.environ.get('SLURM_NTASKS', 1))
    local_rank = int(os.environ.get('SLURM_LOCALID', 0))
    
    # 마스터 주소 설정
    master_addr = os.environ.get('MASTER_ADDR', 'localhost')
    master_port = os.environ.get('MASTER_PORT', '29500')
    
    # 분산 초기화
    dist.init_process_group(
        backend='nccl',
        init_method=f'tcp://{master_addr}:{master_port}',
        world_size=world_size,
        rank=rank
    )
    
    # GPU 설정
    torch.cuda.set_device(local_rank)
    
    return rank, world_size, local_rank

def cleanup_distributed():
    """분산 환경 정리"""
    dist.destroy_process_group()

def main():
    # 분산 설정
    rank, world_size, local_rank = setup_distributed()
    
    if rank == 0:
        print(f"Starting distributed training on {world_size} GPUs")
    
    # 모델 생성 및 GPU로 이동
    model = YourModel().cuda(local_rank)
    model = DDP(model, device_ids=[local_rank])
    
    # 데이터로더 (DistributedSampler 사용)
    train_dataset = YourDataset()
    train_sampler = DistributedSampler(
        train_dataset,
        num_replicas=world_size,
        rank=rank
    )
    train_loader = torch.utils.data.DataLoader(
        train_dataset,
        batch_size=args.batch_size,
        sampler=train_sampler,
        num_workers=4,
        pin_memory=True
    )
    
    # 트레이닝 루프
    for epoch in range(args.num_epochs):
        train_sampler.set_epoch(epoch)  # 중요!
        
        for batch in train_loader:
            # 트레이닝 코드
            pass
        
        # 체크포인트 저장 (rank 0만)
        if rank == 0:
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.module.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
            }, f'/checkpoints/checkpoint_epoch_{epoch}.pt')
    
    cleanup_distributed()

if __name__ == '__main__':
    main()


4단계: 작업 제출 및 모니터링

작업 제출
sbatch distributed_training.sh

작업 상태 확인
squeue -u $USER

실시간 로그 확인
tail -f /fsx/logs/<job_id>.out

GPU 사용률 모니터링 (특정 노드에서)
srun --jobid=<job_id> --nodes=1 --ntasks=1 nvidia-smi


FSx 디렉토리 구조 예시

/fsx/
├── containers/
│   ├── ml-training.sqsh          # 메인 트레이닝 컨테이너
│   ├── preprocessing.sqsh         # 전처리용 컨테이너
│   └── inference.sqsh             # 추론용 컨테이너
├── data/
│   ├── train/
│   ├── validation/
│   └── test/
├── output/
│   ├── 12345/                     # Job ID별 출력
│   └── 12346/
├── checkpoints/
│   ├── 12345/
│   │   ├── checkpoint_epoch_0.pt
│   │   └── checkpoint_epoch_1.pt
│   └── 12346/
└── logs/
    ├── 12345.out
    ├── 12345.err
    ├── 12346.out
    └── 12346.err


주요 체크포인트

컨테이너 준비 시:
- ✅ 충분한 임시 저장 공간 (헤드노드에 50GB+ 권장)
- ✅ ECR 접근 권한 확인
- ✅ Enroot 설치 확인
- ✅ FSx 마운트 상태 확인

분산 트레이닝 시:
- ✅ NCCL 및 EFA 설정
- ✅ DistributedSampler 사용
- ✅ 체크포인트 저장 (rank 0만)
- ✅ 적절한 배치 크기 설정

성능 최적화:
- ✅ Mixed precision training (AMP)
- ✅ Gradient accumulation
- ✅ 데이터 로딩 병렬화
- ✅ FSx Lustre 스트라이핑 설정

이 워크플로우를 통해 ParallelCluster에서 컨테이너 기반 대규모 분산 트레이닝을 효율적으로 수행할 수 있습니다!

> 📝 지식 출처: 이 가이드는 AWS ParallelCluster, Slurm, PyTorch 분산 트레이닝에 대한 일반적인 지식을 바탕으로 작성되었습니다. 최신 ParallelCluster 설정 및 AWS 서비스별 세부 사항은 RAG 모드를 활성화하여 공식 문서를 참조하시기 바랍니다.
